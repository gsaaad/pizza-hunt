...
function getPizza() {
  ...
  // get pizzaInfo based on ID
  fetch(`api/pizzas/${pizzaId}`)
    .then((response) => {
      // if theres no matching ID
      if (!response.ok) {
        throw new Error({ message: "Could not find Pizza, Try again!" });
      }
    ...
    .catch((err) => {
      console.log(err);
      alert("Could not find Pizza with this Id, Taking you back to home page!");
      window.history.back();
    });
}
getPizza();
...
function handleNewReplySubmit(event) {
  ...
  fetch(`/api/comments/${pizzaId}/${commentId}`, {
    ...
    .then((response) => {
      if (!response.ok) {
        throw new Error(
          "There was a problemw ith replying to comment. Try again later"
        );
      }
      response.json();
    })
